...
export function RevealStage({ selectedCards, intention, onRestart, interactionMode, cursorX = 0, cursorY = 0, isHandDetected = false }: RevealStageProps) {
  ...
  const lastHoveredCardRef = useRef<SelectedCard | null>(null);
  const expandedCardIdRef = useRef<number | null>(null);
  ...
  // Gesture-mode collision detection: virtual cursor doesn't fire native mouse events
  useEffect(() => {
    if (interactionMode !== 'gesture' || !isHandDetected) return;

    // If expanded overlay is open, check if finger is over it first
    if (expandedCardData && expandedCardRef.current) {
    ...
    // Check 3 main card containers
    let foundCard: SelectedCard | null = null;
    for (let i = 0; i < selectedCards.length; i++) {
      const el = cardContainerRefs.current[i];
      if (!el) continue;
      const rect = el.getBoundingClientRect();
      if (cursorX >= rect.left && cursorX <= rect.right && cursorY >= rect.top && cursorY <= rect.bottom) {
        foundCard = selectedCards[i];
        break;
      }
    }

    if (foundCard) {
      if (lastHoveredCardRef.current?.card.id !== foundCard.card.id) {
        lastHoveredCardRef.current = foundCard;
        handleCardEnterRef.current(foundCard);
      }
    } else if (lastHoveredCardRef.current) {
      lastHoveredCardRef.current = null;
      handleCardLeaveRef.current();
    }
  }, [cursorX, cursorY, isHandDetected, interactionMode, selectedCards, expandedCardData]);

  // Gesture mode: swipe down to close expanded card
  useEffect(() => {
    if (interactionMode !== 'gesture' || expandedCardId === null || !isHandDetected) return;

    const deltaY = cursorY - lastYRef.current;
    if (deltaY > 40) {
      setExpandedCardId(null);
      setExpandedCardData(null);
    }
    lastYRef.current = cursorY;
  }, [cursorY, interactionMode, expandedCardId, isHandDetected]);

  // Gesture mode: edge scrolling (disabled when expanded card is open)
  useEffect(() => {
    if (interactionMode !== 'gesture' || !isHandDetected || expandedCardData) return;
    ...
    const scroll = () => {
      ...
      if (cy <= 0) { frame = requestAnimationFrame(scroll); return; }

      const threshold = window.innerHeight * 0.15;
      if (cy > window.innerHeight - threshold || cy < threshold) {
        const container = findScrollableParent(rootRef.current);
        if (container) {
          container.scrollBy({ top: cy > window.innerHeight - threshold ? 12 : -12, behavior: 'auto' });
        }
      }
  ...
  }, [interactionMode, isHandDetected, expandedCardData]);
  ...
  // Stable refs for collision detection effect
  const handleCardEnterRef = useRef(handleCardEnter);
  handleCardEnterRef.current = handleCardEnter;
  const handleCardLeaveRef = useRef(handleCardLeave);
  handleCardLeaveRef.current = handleCardLeave;
  ...
  return (
    <div ref={rootRef} className="relative w-full min-h-screen px-4 py-8 pb-32">
      {/* Title */}
      <motion.h2
      ...
      {/* Hover hint — dynamic based on interaction mode */}
      <motion.p
      ...
      >
        {interactionMode === 'gesture'
          ? (isZh ? '指尖悬停于牌面，洞悉原始牌意...' : 'Hover your finger over a card to reveal its primal meaning...')
      ...
      <div ref={exportRef} className="flex flex-col items-center gap-8 p-6 md:p-8 rounded-2xl bg-darkSpace/50 max-w-5xl mx-auto">
          ...
          {selectedCards.map((sc, index) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => { if (interactionMode === 'mouse') handleCardEnter(sc, true); }}
              >
                <span className="text-goldAura/60 text-xs md:text-sm mb-3 tracking-widest">
                ...
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ backfaceVisibility: 'hidden' }}
                    >
                      {/* Full card image — rotated 180° when reversed */}
                      <img
                      ...
                      {/* Transparent glass label at bottom — always upright */}
        ...
        {/* Readings section — contextual summaries + full readings */}
        <div className="flex flex-col gap-4 w-full">
          {selectedCards.map((sc, index) => {
            const isRevealed = revealedCards.includes(index);
            if (!isRevealed) return null;
            const summary = getContextualSummary(sc);
            ...
            return (
              <motion.div
                ...
                className="p-4 md:p-6 rounded-xl bg-mysticPurple/30 border border-goldAura/20 backdrop-blur-sm"
              >
                <h4 className="text-goldAura font-semibold mb-3 tracking-wider text-sm md:text-base"
                    style={{ fontFamily: "'Cinzel', serif" }}>
                ...
                {/* Contextual summary — the only reading in bottom boxes */}
                {summary && (
                  <p className="text-goldAura/90 text-sm md:text-base leading-relaxed px-3 py-2 rounded-lg
        ...
        {expandedCardData && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-sm md:max-w-md rounded-xl border-2 border-goldAura/60
            ...
            >
              {/* Image */}
              <div className="relative w-full aspect-[3/4] max-h-[30vh] overflow-hidden">
                <img
              ...
              <div
                ref={scrollRef}
                className="p-5 max-h-[55vh] overflow-y-auto pb-8"
              >
                ...
                <p className="text-goldAura/70 text-sm leading-relaxed whitespace-pre-line">
      ...
      {/* Edge swipe hint — gesture mode when expanded */}
      {interactionMode === 'gesture' && expandedCardData && (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: [0.3, 0.8, 0.3] }}
          transition={{ repeat: Infinity, duration: 2.5 }}
          className="fixed bottom-20 left-0 right-0 text-center text-goldAura/50 text-sm tracking-widest z-50"
          style={{ fontFamily: "'Cinzel', serif" }}
        >
          {t('edgeSwipeHint')}
        </motion.p>
